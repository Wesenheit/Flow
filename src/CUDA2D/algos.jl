...
@kernel function function_Fluxes(@Const(P::AbstractArray{T}),gamma::T,floor::T,Fxglob::AbstractArray{T},Fyglob::AbstractArray{T}) where T
    #i, j = @index(Global, NTuple)
    ...
    Fx = @localmem eltype(P) (4,N, M)
    Fy = @localmem eltype(P) (4,N, M)
    ...
    if i > 1 && j > 1 && i < Nx && j < Ny
        ...
        if C_max_X < 0 
            for idx in 1:4
                Fx[idx,il,jl] =  FR[idx,il,jl]
            end
        elseif C_min_X < 0 
            for idx in 1:4
                Fx[idx,il,jl] =  FL[idx,il,jl] 
            end
        else
            for idx in 1:4
                Fx[idx,il,jl] = ( FR[idx,il,jl] * C_min_X + FL[idx,il,jl] * C_max_X - C_max_X * C_min_X * (UR[idx,il,jl] - UL[idx,il,jl])) / (C_max_X + C_min_X)
            end
        end
        if C_max_Y <= 0 
            for idx in 1:4
                Fy[idx,il,jl] =  FU[idx,il,jl]
            end
        elseif C_min_Y <= 0 
            for idx in 1:4
                Fy[idx,il,jl] =  FD[idx,il,jl]
            end 
        else
            for idx in 1:4
                Fy[idx,il,jl] = ( FU[idx,il,jl] * C_min_Y + FD[idx,il,jl] * C_max_Y - C_max_Y * C_min_Y * (UU[idx,il,jl] - UD[idx,il,jl])) / (C_max_Y + C_min_Y)
            end
        end
    end
    @synchronize
    for idx in 1:4
        Fxglob[idx,i,j] = Fx[idx,il,jl]
        Fyglob[idx,i,j] = Fy[idx,il,jl]
    end
end
...
function HARM_HLL(comm,P::FlowArr,XMPI::Int64,YMPI::Int64,
    ...
    while t < Tmax
        ...
        @inbounds begin
            Fluxes(P.arr,eos.gamma,floor,Fx.arr,Fy.arr,ndrange = (P.size_X,P.size_Y))
            KernelAbstractions.synchronize(backend)
        ...
        #Calculate Flux
        @inbounds begin
            Fluxes(Phalf.arr,eos.gamma,floor,Fx.arr,Fy.arr,ndrange = (P.size_X,P.size_Y))
            KernelAbstractions.synchronize(backend)
